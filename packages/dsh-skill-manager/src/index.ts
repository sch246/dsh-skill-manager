...
const DEFAULT_SETTINGS: SkillManagerSettings = Object.freeze({ disabled: {} })

/** Fully disabled invocation policy applied by the overrides table. */
const DISABLED_POLICY: Readonly<SkillInvocationPolicy> = Object.freeze({
  modelInvocable: false,
  userInvocable: false,
})

/** Project the settings section onto the registry override table. */
function overridesFrom(settings: SkillManagerSettings): Readonly<Record<string, SkillInvocationPolicy>> {
  const overrides: Record<string, SkillInvocationPolicy> = {}
  for (const [skillName, disabled] of Object.entries(settings.disabled)) {
    if (disabled) overrides[skillName] = DISABLED_POLICY
  }
  return overrides
}

/** Reject disabled keys the schema cannot constrain: names must be skill names. */
function validateSettings(value: SkillManagerSettings): void {
...
export function apply(ctx: Context): void {
  let current: () => SkillManagerSettings = () => DEFAULT_SETTINGS
  const deps: SkillManagerRemoteDeps = {
    current: () => current(),
    setDisabled: async (skillName, disabled) => {
      // Read at call time: the settings provider may attach after this apply.
      const settings = ctx.get('settings')
      if (settings === undefined) throw new Error('skill-manager: settings service is unavailable')
    ...
    commandCtx.commands.register({
      ...
      handler: async ({ agent, rawInput }) => {
        ...
        if (input === 'list') {
          ...
          if (skills.length === 0) return { kind: 'success', text: 'no skills available' }
          return {
            kind: 'success',
            text: skills.map(skill =>
              `- ${skill.name}${disabled[skill.name] === true ? ' (disabled)' : ''}: ${skill.description}`).join('\n'),
          }
        }
